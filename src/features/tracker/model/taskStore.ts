...
import type { TaskState, Task, SimpleTask, RepeatTask, TaskDraft } from './types';
...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  deleteTask: (taskId) =>
    set((store) => {
      const tasks = store.state.tasks.filter((t) => t.id !== taskId).map((t, i) => ({ ...t, position: i }));
      return { state: { ...store.state, tasks } };
    }),
  ...
  toggleSimpleCheck: (taskId) =>
    set((store) => {
      const idx = store.state.tasks.findIndex((t) => t.id === taskId);
      if (idx === -1) return store;
      const task = store.state.tasks[idx];
      if (task.kind !== 'simple') return store;
      const tasks = store.state.tasks.slice();
      tasks[idx] = { ...task, checks: [!task.checks[0]], updatedAt: Temporal.Now.instant().toString() };
      return { state: { ...store.state, tasks } };
    }),

  toggleRepeatCheck: (taskId, index) =>
    set((store) => {
      const idx = store.state.tasks.findIndex((t) => t.id === taskId);
      if (idx === -1) return store;
      const task = store.state.tasks[idx];
      if (task.kind !== 'repeat') return store;
      if (index < 0 || index >= task.checks.length) return store;
      const checks = task.checks.map((v, i) => (i === index ? !v : v));
      const tasks = store.state.tasks.slice();
      tasks[idx] = { ...task, checks, updatedAt: Temporal.Now.instant().toString() };
      return { state: { ...store.state, tasks } };
    }),
}));
